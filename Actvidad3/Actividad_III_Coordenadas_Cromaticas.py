...
##Actividad III: Coordenadas Cromaticas
import cv2
import numpy as np
...
def apply_classifier(img):
    gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
    _, binary = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)  # Usar Otsu para umbral automático
    return binary
...
# Aplicar clasificador a las imágenes en coordenadas cromáticas (con umbral adaptativo)
def apply_chromatic_classifier(img):
    hsv = cv2.cvtColor(img, cv2.COLOR_RGB2HSV)
    _, s_channel, _ = cv2.split(hsv)  # Usar canal de saturación
    _, binary = cv2.threshold(s_channel, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    return binary

binary_chromatic = apply_chromatic_classifier(image_chromatic)
binary_light_chromatic = apply_chromatic_classifier(image_light_chromatic)
binary_dark_chromatic = apply_chromatic_classifier(image_dark_chromatic)
